...
class SwingTrajectoryGenerator:
    def __init__(self, step_height: float) -> None:
        self.step_height = step_height
        self._curve_x = None
        self._curve_y = None
        self._curve_z = None
        self._curve_x_vel = None
        self._curve_y_vel = None
        self._curve_z_vel = None
        self._curve_x_acc = None
        self._curve_y_acc = None
        self._curve_z_acc = None

    def createCurve(self, x0, xf, swing_period):
        half_swing_period = swing_period / 2
        scaling_factor = 1.5

        p1 = x0 + np.array([0., 0., self.step_height / scaling_factor])
        p2 = 0.5 * (x0 + xf) + np.array([0., 0., self.step_height])
        p3 = xf + np.array([0., 0., self.step_height / scaling_factor])

        x = np.array([x0[0], p1[0], p2[0], p3[0], xf[0]])
        y = np.array([x0[1], p1[1], p2[1], p3[1], xf[1]])
        z = np.array([x0[2], p1[2], p2[2], p3[2], xf[2]])
        t = np.array([0, half_swing_period/2, half_swing_period, half_swing_period*3/2, swing_period])
    ...
    def compute_trajectory_references(self, swing_time: float, swing_period: float, lift_off: np.array, touch_down: np.array) -> (np.array, np.array, np.array):
        self.createCurve(lift_off, touch_down, swing_period)
...
if __name__ == "__main__":
    step_height = 0.08
    swing_period = 0.2  # Initial swing period
    trajectory_generator = SwingTrajectoryGenerator(step_height=step_height)
    ...
    for foot_swing_time in np.arange(0.000001, swing_period, 0.002):
        desired_foot_position, \
        desired_foot_velocity, \
        desired_foot_acceleration = trajectory_generator.compute_trajectory_references(swing_time=i, swing_period=swing_period, lift_off=lift_off, touch_down=touch_down)
        i += simulation_dt
